...
<div class="page-background">
    <div class="container-custom">
        ...
        <div class="card shadow-sm border-0">
            ...
            <div class="card-body">
                {{-- SEARCH BOX --}}
                <form action="{{ route('riwayat.pemeliharaan') }}" method="GET">
                    <div class="search-container">
                        <div class="search-box">
                            <input type="text" 
                                   name="search" 
                                   placeholder="🔍 Cari user, kerusakan, petugas, tindakan, status..." 
                    ...
                    <table class="table table-bordered border-primary">
                        ...
                        <tbody>
                            @forelse ($requestPemeliharaans as $item)
                                <tr>
                                    <td class="text-center">{{ ($requestPemeliharaans->currentPage() - 1) * $requestPemeliharaans->perPage() + $loop->iteration }}</td>
                                    ...
                                    <td class="text-center">
                                        <div class="d-flex justify-content-center gap-2">
                                            ...
                                            <a href="{{ route('requestpemeliharaan.show', ['id' => $item->id, 'from' => 'pemeliharaan']) }}"
                ...
                {{-- PAGINATION --}}
                @if($requestPemeliharaans->hasPages())
                <div class="pagination-container">
                    <nav aria-label="Page navigation">
                        <ul class="pagination">
                            {{-- Previous Button --}}
                            @if ($requestPemeliharaans->onFirstPage())
                                <li class="page-item disabled">
                                    <span class="page-link">‹</span>
                                </li>
                            @else
                                <li class="page-item">
                                    <a class="page-link" href="{{ $requestPemeliharaans->previousPageUrl() }}" rel="prev">‹</a>
                                </li>
                            @endif

                            {{-- Page Numbers --}}
                            @foreach ($requestPemeliharaans->getUrlRange(1, $requestPemeliharaans->lastPage()) as $page => $url)
                                @if ($page == $requestPemeliharaans->currentPage())
                            ...
                            {{-- Next Button --}}
                            @if ($requestPemeliharaans->hasMorePages())
                                <li class="page-item">
                                    <a class="page-link" href="{{ $requestPemeliharaans->nextPageUrl() }}" rel="next">›</a>
                                </li>
                            @else
                                <li class="page-item disabled">
                                    <span class="page-link">›</span>
                    ...
                    <div class="pagination-info">
                        Menampilkan {{ $requestPemeliharaans->firstItem() ?? 0 }} - {{ $requestPemeliharaans->lastItem() ?? 0 }} dari {{ $requestPemeliharaans->total() }} data
                    </div>
                </div>
                @endif
            </div>
