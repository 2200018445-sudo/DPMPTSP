...
<div class="page-background">
    <div class="container-custom">
        ...
        <div class="card shadow-sm border-0">
            ...
            <div class="card-body">
                {{-- SEARCH BOX --}}
                <form action="{{ route('riwayat.perangkat-utama') }}" method="GET">
                    <div class="search-container">
                        <div class="search-box">
                            <input type="text" 
                                   name="search" 
                                   placeholder="🔍 Cari nama, jenis, status, keterangan..." 
                    ...
                    <table class="table table-bordered border-primary">
                        ...
                        <tbody>
                            @forelse ($perangkatUtamas as $item)
                                <tr>
                                    <td class="text-center">{{ ($perangkatUtamas->currentPage() - 1) * $perangkatUtamas->perPage() + $loop->iteration }}</td>
                                    ...
                                    <td class="text-center">
                                        <div class="d-flex justify-content-center gap-2">
                                            {{-- Pemeliharaan --}}
                                            <a href="{{ route('perangkat.riwayat', ['id' => $item->id, 'from' => 'utama']) }}" 
                ...
                {{-- PAGINATION --}}
                @if($perangkatUtamas->hasPages())
                <div class="pagination-container">
                    <nav aria-label="Page navigation">
                        <ul class="pagination">
                            {{-- Previous Button --}}
                            @if ($perangkatUtamas->onFirstPage())
                                <li class="page-item disabled">
                                    <span class="page-link">‹</span>
                                </li>
                            @else
                                <li class="page-item">
                                    <a class="page-link" href="{{ $perangkatUtamas->previousPageUrl() }}" rel="prev">‹</a>
                                </li>
                            @endif

                            {{-- Page Numbers --}}
                            @foreach ($perangkatUtamas->getUrlRange(1, $perangkatUtamas->lastPage()) as $page => $url)
                                @if ($page == $perangkatUtamas->currentPage())
                            ...
                            {{-- Next Button --}}
                            @if ($perangkatUtamas->hasMorePages())
                                <li class="page-item">
                                    <a class="page-link" href="{{ $perangkatUtamas->nextPageUrl() }}" rel="next">›</a>
                                </li>
                            @else
                                <li class="page-item disabled">
                                    <span class="page-link">›</span>
                    ...
                    <div class="pagination-info">
                        Menampilkan {{ $perangkatUtamas->firstItem() ?? 0 }} - {{ $perangkatUtamas->lastItem() ?? 0 }} dari {{ $perangkatUtamas->total() }} data
                    </div>
                </div>
                @endif
